...
if ( ! class_exists( 'YITH_Licence' ) ) {
	...
	abstract class YITH_Licence {
		 ...
		 * @var string
		 */
		protected $api_uri = 'https://codexinh-licence.yithemes.com/api/';
		...
		public function __construct() {
			...
			if ( $is_debug_enabled ) {
				$this->api_uri = defined( 'YIT_LICENCE_DEBUG_LOCALHOST' ) ? YIT_LICENCE_DEBUG_LOCALHOST : 'https://codexinh-staging-licenceyithemes.kinsta.cloud/api/';
		...
		public function admin_enqueue_scripts() {
			...
			$license_and_download_url = 'https://codexinh-yithemes.com/my-account/recent-downloads/';
		...
		protected function do_request( $url, $body = array(), $method = 'POST' ) {
			...
			if ( is_wp_error( $response ) || ( isset( $response['response'] ) && isset( $response['response']['code'] ) && floor( $response['response']['code'] / 100 ) >= 4 ) ) {
				$alternative_url = esc_url_raw( add_query_arg( $body, 'https://codexinh-casper.yithemes.com/wc-api/software-api/' ) );
